	...
	public void render(GameState state, PerspectiveCamera camera) {
		for (Planet planet : state.planets) {
			if (planet.defense != null) {
				...
				armorBar.setCurrentValue(planet.defense.currentArmor);
				...
				shieldBar.setCurrentValue(planet.defense.currentShield);
		...
		for (Ship ship : state.ships) {
			if (ship.defense != null) {
				...
				PlainRectangularLoadingBar armorBar = createBar(Color.ORANGE);
				armorBar.setBounds(new Rectangle(screenPos.x - 9, screenPos.y - 16, 18, 2));
				armorBar.setCurrentValue(ship.defense.currentArmor);
				armorBar.setMaxValue(ship.defense.armor);
				armorBar.render();
				
				PlainRectangularLoadingBar shieldBar = createBar(new Color(0, 0.8f, 0.8f, 1f));
				shieldBar.setBounds(new Rectangle(screenPos.x - 9, screenPos.y - 16, 18, 2));
				shieldBar.setCurrentValue(ship.defense.currentShield);
